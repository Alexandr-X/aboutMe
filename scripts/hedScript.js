...
addEventListener('click',(event)=>{
    ...
    for(let i = 0;i<arrNavig.length;i++){
        if(event.target === arrNavig[i]){
            switch(i){
                case 0: let text = document.querySelector('.textCont');
                text.scrollIntoView({behavior:"smooth",block: "center", inline: "center"});
                break;
                case 1: let skill = document.querySelector('.htmlCont');
                skill.scrollIntoView({behavior:"smooth",block: "end", inline: "center"});
                break;
                case 2: let proj = document.querySelector('.linkPoroj');
                proj.scrollIntoView({behavior:"smooth",block: "center", inline: "center"});
                break;
                case 3: let linke = document.querySelector('footer');
                linke.scrollIntoView({behavior:"smooth",block: "center", inline: "center"});
                break;
            }
        }
    }
